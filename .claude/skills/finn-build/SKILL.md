...
Using the Linear connector, list issues on team `KOI` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `KOI-NNN-short-slug`, using the issue's real identifier.
...
- `Closes KOI-NNN`, using the real Linear issue identifier
